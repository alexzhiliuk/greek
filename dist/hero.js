...
//   history.scrollRestoration = 'manual'; // отключаем восстановление позиции
// }
// window.scrollTo(0, 0);
// document.body.style.overflow = 'hidden';
document.addEventListener("visibilitychange", () => {
  if (document.visibilityState === "visible") {
    // 🔒 Блокируем всё, чтобы ничего не сработало в процессе сброса
    document.body.style.overflow = "hidden";

    // 🧹 Удаляем все старые ScrollTrigger
    ScrollTrigger.getAll().forEach(trigger => trigger.kill());

    // 🕒 Небольшая задержка, чтобы DOM стабилизировался
    setTimeout(() => {
      // ⬆️ Скроллим страницу в самый верх
      window.scrollTo({ top: 0, behavior: "instant" || "auto" });

      // 🔄 Сбрасываем анимационные блоки и состояния
      gsap.globalTimeline.clear(); // очищаем любые висячие анимации
      updateFrame(0); // ставим первый кадр на канвас
      if (frames[0]) ctx.drawImage(frames[0], 0, 0, canvas.width, canvas.height);

      // 🔁 Пересоздаём ScrollTrigger
      initScrollTrigger();

      // 🧩 Обновляем GSAP-расчёты
